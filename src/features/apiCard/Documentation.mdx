...
class="text-nowrap">/{props.module.id}/jobs</code> in the [API documentation]({props.baseUrl}/docs)
for additional information about the names and types of all parameters available.
...
For larger jobs, you need to use a POST request to the <code class="text-nowrap">/{props.module.id}/jobs</code> endpoint
and provide the input molecules in the request body. 
...
To upload an input file, you need to use a POST request to the <code class="text-nowrap">/{props.module.id}/jobs</code> 
endpoint and include the file in the request body.
...
See all endpoints at [{props.baseUrl}/docs]({props.baseUrl}/docs).